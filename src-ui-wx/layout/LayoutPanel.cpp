...
void LayoutPanel::OnPreviewRightDown(wxMouseEvent& event)
{
    ...
    mnuAILayout->Append(ID_PREVIEW_AI_PROP_DESIGNER, _("AI Custom Prop Designer (2-Opt TSP & Batch)..."));
...
void LayoutPanel::OnPreviewModelPopup(wxCommandEvent& event)
{
    if (event.GetId() == ID_PREVIEW_RESET) {
        ...
        dlg.ShowModal();
    } else if (event.GetId() == xLightsFrame::ID_MENUITEM_AI_PHOTO_3D_RECONSTRUCTOR ||
               event.GetId() == xLightsFrame::ID_MENUITEM_AI_SUBMODEL_DETECTOR ||
               event.GetId() == xLightsFrame::ID_MENUITEM_AI_AUDIENCE_SIGHTLINE_OPTIMIZER ||
               event.GetId() == xLightsFrame::ID_MENUITEM_AI_POWER_INSPECTOR ||
               event.GetId() == xLightsFrame::ID_MENUITEM_AI_MODEL_MAPPER) {
        wxCommandEvent cmd(wxEVT_COMMAND_MENU_SELECTED, event.GetId());
...
void LayoutPanel::OnItemContextMenu(wxTreeListEvent& event)
{
    ...
    mnuAILayout->Append(xLightsFrame::ID_MENUITEM_AI_POWER_INSPECTOR, _("AI Power Injection Inspector..."));
    mnuAILayout->Append(xLightsFrame::ID_MENUITEM_AI_MODEL_MAPPER, _("AI Model Mapping Wizard..."));
    mnuContext.AppendSubMenu(mnuAILayout, _("🎨 AI Layout & Prop Copilot"));
