...
std::string VideoSequenceEmulator::ExportPlanToXsqXml(const VideoSequencePlan& plan, int sequenceDurationMs) {
    ...
    // Collect and index unique effect settings & palettes
    std::vector<std::string> effectDB;
    std::map<std::string, int> effectRefMap;

    std::vector<std::string> colorPalettes;
    std::map<std::string, int> paletteRefMap;

    for (const auto& cue : plan.generatedCues) {
        std::string settings = cue.effectSettings.empty() ? "B_CHOICE_BufferStyle=Default" : cue.effectSettings;
        if (effectRefMap.find(settings) == effectRefMap.end()) {
            effectRefMap[settings] = (int)effectDB.size();
            effectDB.push_back(settings);
        }

        std::string pal = "C_BUTTON_Palette1=" + cue.primaryColor + ",C_BUTTON_Palette2=" + cue.secondaryColor +
                          ",C_CHECKBOX_Palette1=1,C_CHECKBOX_Palette2=1";
        if (paletteRefMap.find(pal) == paletteRefMap.end()) {
            paletteRefMap[pal] = (int)colorPalettes.size();
            colorPalettes.push_back(pal);
        }
    }
    ...
    oss << "<xsequence BaseChannel=\"0\" ChanCtrlBasic=\"0\" ChanCtrlColor=\"0\" "
        << "FixedPointTiming=\"1\" ModelBlending=\"true\" sequenceTiming=\"50\" "
    ...
    oss << "    <sequenceTiming>50 ms</sequenceTiming>\n";
    oss << "    <sequenceDuration>" << std::fixed << std::setprecision(3) << (totalDuration / 1000.0) << "</sequenceDuration>\n";
    oss << "  </head>\n";

    // ColorPalettes section
    oss << "  <ColorPalettes>\n";
    for (const auto& pal : colorPalettes) {
        oss << "    <ColorPalette>" << pal << "</ColorPalette>\n";
    }
    oss << "  </ColorPalettes>\n";

    // EffectDB section
    oss << "  <EffectDB>\n";
    for (const auto& effSettings : effectDB) {
        oss << "    <Effect>" << effSettings << "</Effect>\n";
    }
    oss << "  </EffectDB>\n";
    ...
    // ElementEffects section
    oss << "  <ElementEffects>\n";
    for (const auto& [modelName, cues] : cuesByModel) {
        oss << "    <Element type=\"model\" name=\"" << modelName << "\">\n";
        oss << "      <EffectLayer>\n";
        for (const auto& c : cues) {
            std::string settings = c.effectSettings.empty() ? "B_CHOICE_BufferStyle=Default" : c.effectSettings;
            std::string pal = "C_BUTTON_Palette1=" + c.primaryColor + ",C_BUTTON_Palette2=" + c.secondaryColor +
                              ",C_CHECKBOX_Palette1=1,C_CHECKBOX_Palette2=1";
            int refIdx = effectRefMap[settings];
            int palIdx = paletteRefMap[pal];

            oss << "        <Effect ref=\"" << refIdx << "\" palette=\"" << palIdx << "\" "
                << "name=\"" << c.effectType << "\" "
